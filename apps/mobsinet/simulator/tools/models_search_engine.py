...
class ModelsSearchEngine:
    @staticmethod
    def find_model(model_arg: str, model_type: Literal['connectivity', 'mobility', 'interference', 'reliability', 'distribution', 'message_transmission']) -> Type['AbcModel']:
        """
        Finds a model from a given string and type.

        Args:
            model_arg (str): The name of the model to find.
            model_type (Literal['connectivity', 'mobility', 'interference', 'reliability', 'distribution', 'message_transmission']):
                The type of the model to find.

        Returns:
            Type[AbcModel]: The found model.

        Raises:
            ModuleNotFoundError: If the model is not found.
        """
        if (model_type == 'connectivity'):
            return ModelsSearchEngine.find_connectivity_model(model_arg)
        elif (model_type == 'mobility'):
            return ModelsSearchEngine.find_mobility_model(model_arg)
        elif (model_type == 'interference'):
            return ModelsSearchEngine.find_interference_model(model_arg)
        elif (model_type == 'reliability'):
            return ModelsSearchEngine.find_reliability_model(model_arg)
        elif (model_type == 'distribution'):
            return ModelsSearchEngine.find_distribution_model(model_arg)
        elif (model_type == 'message_transmission'):
            return ModelsSearchEngine.find_message_transmission_model(model_arg)
